<script>
	...
	import TabsCard from '$lib/components/dashboard/TabsCard.svelte';
	...
	// VÃ©rifier si le passage a commencÃ©
	$: passageStarted = selectedTournage?.start_time
		? new Date(selectedTournage.start_time) <= currentTime
		: false;

	// VÃ©rifier si le passage est terminÃ©
	$: passageEnded = selectedTournage?.end_time
		? new Date(selectedTournage.end_time) < currentTime
		: false;

	$: uploadDisabled =
		!selectedTournage?.drive ||
		selectedTournage?.status === 'submitted' ||
		selectedTournage?.status === 'absent' ||
		!passageStarted;

	async function acceptCgu() {
		if (!selectedTournage || selectedTournage.cgu === 1 || isUpdatingCgu) return;

		isUpdatingCgu = true;
		const formData = new FormData();
		formData.append('passage_id', selectedTournage.passage_id);

		try {
			const response = await fetch('?/acceptCgu', {
				method: 'POST',
				body: formData
			});

			if (response.ok) {
				selectedTournage.cgu = 1;
			} else {
				console.error('Erreur action acceptCgu', await response.text());
			}
		} catch (error) {
			console.error('Erreur lors de la mise Ã  jour des CGU', error);
		} finally {
			isUpdatingCgu = false;
		}
	}
	...
	function getStatusInfo(t) {
		if (!t) return { label: 'Inconnu', color: 'bg-secondary' };
		const now = new Date();
		const start = t.start_time ? new Date(t.start_time) : null;
		const end = t.end_time ? new Date(t.end_time) : null;

		if (t.status === 'submitted') return { label: 'Rendu', color: 'bg-purple' };
		if (t.status === 'absent') return { label: 'Absent', color: 'bg-danger' };

		if (!start || !end) return { label: 'En attente', color: 'bg-secondary' };
		if (now > end) return { label: 'TerminÃ©', color: 'bg-success' };
		if (now >= start && now <= end) return { label: 'En cours', color: 'bg-primary' };
		return { label: 'PrÃªt', color: 'bg-info' };
	}
	...
	function formatDateTime(value) {
		return value ? new Date(value).toLocaleString() : 'Non dÃ©fini';
	}
	...
	function getTimingInfo(t) {
		...
		const end = t.end_time ? new Date(t.end_time) : null;
		if (!start || !end) {
			return { text: 'Planning non dÃ©fini', clockTime: null };
		}
		...
		return { text: `TerminÃ© il y a ${formatDuration(-toEnd)}`, clockTime: null };
	}
</script>
		...
		<div class="page-body">
			<div class="container-xl my-auto">
				<div class="empty">
					...
					<p class="empty-title">Aucun tournage trouvé</p>
					<p class="empty-subtitle text-secondary">
						Vous n'êtes affecté à aucun tournage pour le moment.
					</p>
				</div>
			</div>
		</div>
			...
			<div class="container-xl">
				<div class="row g-2 align-items-center">
					<div class="col">
						...
						<h2 class="page-title">{selectedTournage?.name}</h2>
						<div class="text-secondary">Créé le {formatDate(selectedTournage?.created_at)}</div>
					</div>
					<div class="col-auto ms-auto d-print-none">
						{#if hasMultiple}
							<select class="form-select" bind:value={selectedId}>
								{#each tournages as t}
			...
			<div class="container-xl">
				<div class="row row-deck row-cards">
					...
					<div class="col-sm-6 col-lg-3">
						<TimerCard tournage={selectedTournage} {passageStarted} {passageEnded} {timingInfo} />
					</div>
					<div class="col-sm-6 col-lg-3">
						<InstructionsCard cgu={selectedTournage.cgu} {isUpdatingCgu} on:accept={acceptCgu} />
					</div>
					{#if selectedTournage.cgu === 1}
						<div class="col-12">
							<TabsCard
								...
								{canDownloadPrevious}
								{previousProject}
								uploadDisabled={!passageStarted || passageEnded}
							/>
						</div>
					{/if}
				</div>
